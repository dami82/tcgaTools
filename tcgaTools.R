...
#
# Damiano Fantini
# Collection of tools for Wang L et al, 2017
# 
# check and load libraries
my.libs <- c("TCGAretriever", "survival", "gplots", "ggplot2", "GO.db",
             "OrganismDbi","org.Hs.eg.db","BSgenome.Hsapiens.UCSC.hg19","Homo.sapiens")
if (sum(my.libs %in% rownames(installed.packages())) != length(my.libs)) {
  message("Some libraries are missing!")
  message("Please, make sure to install the following packages from CRAN/bioconductor before proceeding..")
  for (tmp.lib in my.libs[!my.libs %in% rownames(installed.packages())]) {
    message(paste("--> '", tmp.lib, "' is missing!", sep = ""))
  }
} else {
...
fetch_and_prepare_from <- function(where = "cbio",
  ...
  if (where == "cbio" & is.character(data) & length(data) == 1){
    ...
    seq.case.id <- grep ("sequenc|mutation", seq.case.id, value = TRUE, ignore.case = TRUE)[1]
    #
    tmp.mut.profi <- get_genetic_profiles(data)$genetic_profile_id
    tmp.mut.profi <- grep("sequenc|mutation", tmp.mut.profi, value = TRUE, ignore.case = TRUE)[1]
    #
    ...
    tmp.mut.dset <- data.frame(get_ext_mutation(case_id = seq.case.id,
                                                ...
                                                glist = gene.symbol), stringsAsFactors = FALSE)
    ...
    #
    # remove if unused
    # bckup.mut.tcga.ids <- unique(grep("TCGA", tmp.mut.dset$case_id, value = TRUE))
    #
    case.to.exclude <- c() #place holder for vector including cases to exclude
    #
  } else if (where == "raw_data") {
    if (!(is.data.frame(data) &
          sum(c("SYMBOL", "SAMPLE", "type", "Protein_Change") %in% colnames(data)) == 4 &
          ! is.null(clin.data) )) {
    ...
    tmp.mut.dset$Protein_Change <- gsub("\\*{2}", "*", gsub("fs$", "*fs", tmp.mut.dset$Protein_Change))
    #
    case.to.exclude <- unique(tmp.mut.dset[grep("Splice", tmp.mut.dset$type) , ]$SAMPLE)
...
)
{
  ...
  if (! (is.character(method) & length(method) == 1 & method %in% supported_methods))
    stop (paste("Unsupported method. The supported methods are:", paste(supported_methods, collapse = ", ")))
  #
  # -------------------------- data are now ready for calling the mutations -----------------------
  #
  #
  if (method == "missense.only") {
    ex.idx <- grep("missense", tmp.mut.dset$mutation_type, ignore.case = TRUE, invert = TRUE)
    ...
    #
    tmp.mut.dset$amino_acid_change <- gsub("(\\*).*$", "*", tmp.mut.dset$amino_acid_change)
    tmp.mut.dset$amino_acid_change <- gsub("^.*(splice).*$", "", tmp.mut.dset$amino_acid_change)
    exluded.cases <- unique(tmp.mut.dset[tmp.mut.dset$amino_acid_change == "",]$case_id)
    tmp.mut.dset <- tmp.mut.dset[tmp.mut.dset$amino_acid_change != "",]
    #
    tmp.mut.dset$range.status <- sapply(tmp.mut.dset$amino_acid_change, (function(aach){
      if (window.min < 0 | window.max < 0) {
        ...
        if(regexpr("\\*", aach) > 0) {
          ...
          if(is.na(as.integer(gsub("[[:alpha:]]|\\*|\\-|\\_", "", aach)))) {
            "exclude"
          } else if(as.integer(gsub("[[:alpha:]]|\\*|\\-|\\_", "", aach)) < (window.max + 1)) {
            "affected"
          } else {
            "OK"
          }
  ...
  tmp.mut.dset$position <- suppressWarnings(as.integer(gsub("[[:alpha:]]|\\*|\\.", "", tmp.mut.dset$amino_acid_change)))
  #
  ...
  tmp.mut.dset$inside <- tmp.mut.dset$position > window.min & tmp.mut.dset$position < window.max
  ...
  if (method == "missense.only") {
    ...
    exluded.cases <- unique(c(exluded.cases, names(case.report[is.na(case.report)])))
  ...
  case.bckground <- unique(tot.cases[! tot.cases %in% c(case.in, case.out, case.exclude)])
  #
  #
  # Handle case lists
  #
...
call_survival_by_range <- function(mutcases.list)
{
  ...
  if (data.type == "raw") {
    ...
    tmp.surv$OS_STATUS <- gsub("(^.*)dea(.*$)","DECEASED",tmp.surv$survival_status)
    ...
    if (sum(c("CASE_ID", "OS_MONTHS", "OS_STATUS") %in% colnames(clin.data)) == 3) {
      clin.data <- clin.data[,c("CASE_ID", "OS_MONTHS", "OS_STATUS")]
      clin.data <- clin.data[clin.data$CASE_ID %in% c(case.in, case.out, case.bckground),]
      clin.data$group <- "wt"
...
merge_km_data <- function(km.data.list) {
  ...
  if (sum(my.symbols != top.gene) > 0) {
    message(paste(sum(my.symbols != top.gene), "datasets were excluded because different gene.symbols were tested..."))
  ...
  if (sum(my.methods != top.method) > 0) {
    message(paste(sum(my.methods != top.method), "datasets were excluded because different analys methods were used..."))
...
)
{
  ...
  # And then, plot
  if (main == "") {
    main <- gsub("[[:blank:]]{2}", " ", paste("Survival by", gene.symbol ,"mutation status"))
  }
  if (!is.null(xlim)) {
    x.max <- max(xlim) 
  } else {
  ...
  if (survival.mutation.data$method == "missense.only"){
    ...
    #
    custom.labs <- rev(c(paste(gene.symbol, "wt"), "Mut out of Domain", "Domain Mut/Lost"))
    if (cust.mll3.lab) {
...
#
get_hotspot_mutations <- function(gene.symbol, study.id = "all", precise = TRUE, freq = FALSE) {
  if(study.id[1] == "all") {
  ...
  if (precise == FALSE) {
    ...
    my.cds.tx.id <- suppressMessages(OrganismDbi::select(Homo.sapiens::Homo.sapiens, keys = gene.symbol, keytype = "SYMBOL", columns = c("CDSID", "ENTREZID", "TXID")))
    ...
    iso.lens <- do.call(base::c, lapply(all.tx, (function(tx.lst){
      cds.seq <- suppressMessages(Biostrings::getSeq(BSgenome.Hsapiens.UCSC.hg19::BSgenome.Hsapiens.UCSC.hg19,
                                                     my.cds[tx.lst$CDSID]))
...
plot_binned_mutcounts <- function(mut.binned, mut.type = "both", colors = c("#2166ac", "#f63535"), main = "", ylim = NULL) {
  # prepare data
  ...
  my.mat.1 <- cbind(c(1,1:length(mtbn.all.bin),length(mtbn.all.bin)),
                    as.integer(c(1,seq(1, length(mtbn.all.ori), along.with = mtbn.all.bin),
                                 length(mtbn.all.ori))),
                    c(0,mtbn.all.bin,0),
                    rep(1, length(mtbn.all.bin) +2))
  my.mat.2 <- cbind(c(1,1:length(mtbn.mis.bin),length(mtbn.mis.bin)),
                    as.integer(c(1,seq(1, length(mtbn.mis.ori), along.with = mtbn.mis.bin),
                                 length(mtbn.mis.ori))),
                    c(0,mtbn.mis.bin,0),
  ...
  #
  # Plot
  plt <- ggplot(my.df, aes(x = factor(aa.pos), y = counts, fill = as.factor(group)))
  ...
  #
  tick.vect <- c(5,10,25,50,100,200,250,300,500,1000,2000,5000,10000,20000,25000,50000,100000,200000)
  ...
  #
  plt <- plt + scale_x_discrete(breaks = my.brks, labels = my.brk.labs, expand = c(0,0))
  if (!is.null(ylim)) {
    plt <- plt + scale_y_continuous(limits = ylim,  expand = c(0,0))
  } else {
    plt <- plt + scale_y_continuous(limits = c(0, max(my.df$counts) * 1.2),  expand = c(0,0))
  }
  #
...
#
barplot.gene <- function(gene.freq.mutat, min.color = "#deebf7", max.color = "#08306b", ylim = NULL, gene.symbol = NULL){
  test.my.gene <- gene.freq.mutat
  my.colors <- colorRampPalette(c(min.color, max.color))(max(as.numeric(test.my.gene$mut.count))+2)
  pl.dim <- barplot(test.my.gene$ratio, col = my.colors[test.my.gene$mut.count], 
                    ylab = "Freq. of Tumors with Mutation", ylim = ylim,
                    main = paste(gene.symbol, "mutations in TCGA datasets"), names.arg = rownames(test.my.gene), las = 2)
  # key legend
  min.x <- min(pl.dim[,1])
  max.x <- max(pl.dim[,1])
  if (!is.null(ylim)){
...
prep_custom_oncoprint <- function(csid = "brca_tcga",
  ...
  cont.tab <- data.frame(matrix(0, nrow = length(tcga.id.all), ncol = 6), row.names = tcga.id.all)
  ...
  tab.9g[3,1] <- sum(apply(cont.tab[,1:2], 1, sum) > 0 & apply(cont.tab[,4:6], 1, sum) == 0)
  #
  ...
  #
  tab.9g[1,3] <- sum(apply(cont.tab[,1:3], 1, sum) == 0 & apply(cont.tab[,4:5], 1, sum) > 0)
  tab.9g[2,3] <- sum(cont.tab[,3] == 1 & apply(cont.tab[,4:5], 1, sum) > 0)
  tab.9g[3,3] <- sum(apply(cont.tab[,1:2], 1, sum) > 0 & apply(cont.tab[,4:5], 1, sum) > 0)
  #
  # assemble and return
  final.out <- list()
...
plot_custom_oncoprint <- function(oncoprint.df, trim.wt = TRUE) {
  ...
  #
  #
  pl <- ggplot(my.df, aes(x=id, y=gene)) + geom_tile(fill="white", colour="white", size=1.1) + 
    ...
    geom_tile(data=my.df, aes(x=id, y=gene), inherit.aes=FALSE, width=.9, height=.9, fill="gray85", colour=NA, size=2) + 
    geom_tile(data=my.df[my.df$del == "DEL",], aes(x=id, y=gene), inherit.aes=FALSE, width=.9, height=.9, fill="#377eb8", colour=NA, size=2) + 
    geom_tile(data=my.df[my.df$mut.stat == "LOST",], aes(x=id, y=gene), inherit.aes=FALSE, width=square_w, height=square_h, fill="#e41a1c") + 
    geom_tile(data=my.df[my.df$mut.stat == "MUT",], aes(x=id, y=gene), inherit.aes=FALSE, width=square_w, height=square_h, fill="#ff7f00") +
    theme_minimal() + xlab("") + ylab("") + 
    theme(panel.background = element_blank(), line = element_blank(), axis.text.x = element_blank(), 
...
std_2gene_oncoprint <- function(csid = "nsclc_tcga_broad_2016",
  ...
  mut.profi <- grep("mutations$", get_genetic_profiles(csid = csid)[,1], value = TRUE)
  cna.profi <- grep("_gistic$", get_genetic_profiles(csid = csid)[,1], value = TRUE)
  if(length(cna.profi) == 0)
    cna.profi <- grep("_cna$", get_genetic_profiles(csid = csid)[,1], value = TRUE)
  #
  ...
  square_h = 0.6
  square_w = 0.5
  gpp <- ggplot(my.df, aes(x=id, y=gene)) + geom_tile(fill="white", colour="white", size=1.1) +
    scale_y_discrete(breaks=my.genes, limits = rev(my.genes)) +
    scale_x_discrete(limits = unique(my.df$id[order(my.df$score, decreasing = TRUE)])) +
    geom_tile(data=my.df, aes(x=id, y=gene), inherit.aes=FALSE, width=.8, height=.9, fill="gray85", colour=NA, size=2) +
    geom_tile(data=my.df[my.df$del == "DEL",], aes(x=id, y=gene), inherit.aes=FALSE, width=.9, height=.9, fill="#345fdf", colour=NA, size=2) +
    
    geom_tile(data=my.df[my.df$mut == "NSN",], aes(x=id, y=gene), inherit.aes=FALSE, width=square_w, height=square_h, fill="#1a1c1b") +
    geom_tile(data=my.df[my.df$mut == "IND",], aes(x=id, y=gene), inherit.aes=FALSE, width=square_w, height=square_h, fill="#feb24c") +
    geom_tile(data=my.df[my.df$mut == "MIS",], aes(x=id, y=gene), inherit.aes=FALSE, width=square_w, height=square_h, fill="#35ae5d") +
    theme_minimal() + xlab("") + ylab("") +
    theme(panel.background = element_blank(), line = element_blank(), axis.text.x = element_blank(),
  ...
  tab.4g[2,1] <- sum(apply(cont.tab[,1:4], 1, sum) > 0 & apply(cont.tab[,5:8], 1, sum) == 0)
  tab.4g[1,2] <- sum(apply(cont.tab[,1:4], 1, sum) == 0 & apply(cont.tab[,5:8], 1, sum) > 0)
  tab.4g[2,2] <- sum(apply(cont.tab[,1:4], 1, sum) > 0 & apply(cont.tab[,5:8], 1, sum) > 0)
  #
  # assemble and return
  final.out <- list()
  final.out[["plot"]] <- gpp
