...
```{r survival analysis b1, echo=T, message=F, eval=T, fig.align="center", size = "in", fig.width=5.8, fig.height=5.1}
...
Done, **success!** This document covered and described the R code used for TCGA data analyses included in Wang L et al, 2017. These lines of code can be used for reproducing the plots included in the publication or adapted for running similar analyses. For questions, email \url{"damiano.fantini@gmail.com"}

\newpage
## Appendix 1: Custom functions - Code
The analyses described in this Vignette rely on some published libraries as well as on a set of custom in-house functions that were not published before. The source code of these custom functions is available at the following URL: \url{"http://www.labwizards.com/rlib/tcgaTools.R"}. The R code of the custom in-house functions is provided below.
